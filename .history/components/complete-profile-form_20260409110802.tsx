...
export function CompleteProfileForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.replace(isAdmin ? "/admin/events" : "/events");
    } catch (submitError: unknown) {
      const message = (() => {
        if (submitError && typeof submitError === "object") {
          const maybeError = submitError as {
            message?: string;
            details?: string;
            hint?: string;
            code?: string;
          ...
          if (maybeError.message) {
            const context = [maybeError.details, maybeError.hint, maybeError.code]
              .filter(Boolean)
              .join(" | ");
            return context ? `${maybeError.message} (${context})` : maybeError.message;
          }
        }

        return submitError instanceof Error ? submitError.message : "An error occurred";
      })();
      ...
      if (normalizedMessage.includes("duplicate key")) {
        ...
        normalizedMessage.includes("on_conflict") ||
        normalizedMessage.includes("schema")
      ) {
        setError(
          "Database schema mismatch detected for profiles table. Please run the migration SQL shared by the developer.",
        );
      } else {
        setError(message);
      }
